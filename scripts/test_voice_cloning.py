...
try:
    ...
    print("ERROR: soundfile/numpy not found.")
    sys.exit(1)
...
def prepare_audio(source_path: pathlib.Path, output_path: pathlib.Path) -> bool:
    """Convert audio to 24kHz mono WAV using ffmpeg if not already in that format."""
    try:
        data, samplerate = sf.read(str(source_path))
        if samplerate == 24000 and (data.ndim == 1 or data.shape[1] == 1):
...
def transcribe_audio(audio_path: pathlib.Path) -> str:
    """Use Whisper to transcribe reference audio."""
    print(f"  Transcribing reference audio with Whisper (base model)...")
...
def clone_and_synthesize(tts_model, reference_audio: pathlib.Path,
...
def run_tests():
    ...
    sample_files = [
        f for f in SAMPLE_AUDIO_DIR.iterdir()
        if f.suffix.lower() in audio_extensions and f.name != ".gitkeep"
    ]
    ...
    for ref_file in sample_files:
        ...
        print(f"  Preparing audio (24kHz mono WAV)...")
        ...
        for i, target_text in enumerate(TARGET_TEXTS):
            ...
            try:
                result = clone_and_synthesize(
                    tts_model, prepared_path, reference_text, target_text, output_path
                )

...
1. **`main` ブランチは常にデプロイ可能な状態を保つ**
   - `main` への直接コミットは禁止
   - `main` にマージされたコードは動作が保証された状態であること

2. **作業は必ずフィーチャーブランチで行う**
   - `main` から分岐する
   - 1ブランチ = 1 Issue（またはまとまった1つの作業）

3. **PR（Pull Request）は必須**
   - すべての変更は PR 経由で `main` にマージする
   - セルフレビューでも PR を立てる（作業記録・変更の可視化のため）
   - PR 本文に対応 Issue を `Closes #N` で明記する

4. **PR はレビュー・承認後にマージする**
   - CI（pre-commit）が通っていることを確認する
   - 問題がなければ Squash merge または Merge commit でマージする

5. **マージ後はブランチを削除する**
   - `main` にマージされたブランチは速やかに削除する
...
**例:**
```
feature/issue-1-fix-torchaudio-cuda
fix/issue-2-transformers-cache-warning
docs/issue-12-validation-results
chore/issue-N-update-pre-commit
```
